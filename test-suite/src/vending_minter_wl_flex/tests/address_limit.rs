use crate::common_setup::{
    ...
    setup_minter::{
        ...
        vending_minter::setup::vending_minter_wl_flex_code_ids,
    },
    templates::{vending_minter_wl_flex_per_address_limit, vending_minter_wl_flex_template},
};
use crate::common_setup::{
...
fn check_per_address_limit() {
    let vt = vending_minter_wl_flex_template(2);
...
fn check_per_address_limit_3_percent() {
    let vt = vending_minter_wl_flex_template(200);
    let (mut router, creator, buyer) = (vt.router, vt.accts.creator, vt.accts.buyer);
    let minter_addr = vt.collection_response_vec[0].minter.clone().unwrap();
    // Set to genesis mint start time
    setup_block_time(&mut router, GENESIS_MINT_START_TIME, None);
...
//     let vt = vending_minter_wl_flex_template(25);
...
//     let vt = vending_minter_wl_flex_template(215);
//     let (mut router, creator, buyer) = (vt.router, vt.accts.creator, vt.accts.buyer);
//     let minter_addr = vt.collection_response_vec[0].minter.clone().unwrap();
//     // Set to genesis mint start time
//     setup_block_time(&mut router, GENESIS_MINT_START_TIME, None);
...
fn check_per_address_limit_above_max_per_address_limit() {
    let vt = vending_minter_wl_flex_template(5000);
    let (mut router, creator, _) = (vt.router, vt.accts.creator, vt.accts.buyer);
    let minter_addr = vt.collection_response_vec[0].minter.clone().unwrap();
    // Set to genesis mint start time
    setup_block_time(&mut router, GENESIS_MINT_START_TIME, None);
...
fn check_invalid_update_per_address_limit() {
    ...
    let code_ids = vending_minter_wl_flex_code_ids(&mut router);
...
fn check_invalid_instantiate_per_address_limit() {
    ...
    let vt = vending_minter_wl_flex_per_address_limit(num_tokens, per_address_limit);
